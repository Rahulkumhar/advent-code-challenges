...
// each simultaneously choose one of Rock, Paper, or Scissors using a hand shape.
...
const rounds = smallInput.split(/\n/);

rounds.forEach((round) => {
  const oponentHand = round[0];
  const ourHand = round[2];

  totalPoints = totalPoints + rules[oponentHand][ourHand];
});
...
rounds.forEach((round) => {
  ...
  totalPoints2 = totalPoints2 + part2Rules[oponentHand][ourHand];
});
